...
export default class ChatClientHelper {
  ...
  login(identity, pushChannel, registerForPushCallback, showPushCallback) {
    let that = this;
    const URL = this.host + '/chat-client-configuration.json';
    return fetch(URL, { headers: this.basicAuthHeaders })
      .then((response) => {
        ...
        console.log('error in fetching chat config', err);
      })
      .catch((err) => {
        console.warn('err coming', err);
